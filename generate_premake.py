...
def premake_config_includes(configs):
    lines = []
    for config in configs:
        lines.append(premake_filter([config.premake_filters]))
        if config.os == 'windows':
            lines.append(f'    buildoptions({{ "/FI{config.config_h}" }})\n')
        else:
            lines.append(f'    buildoptions({{ "-include {config.config_h}" }})\n')
    lines.append(premake_filter())
    return ''.join(lines)

config_includes = premake_config_includes(supported_configs)
templates = {}
